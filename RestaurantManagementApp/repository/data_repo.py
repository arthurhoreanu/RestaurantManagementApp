...
class DataRepo:
    ...
    def read_file(self):
        with open(self._file, 'r') as input_file:
            dictionary_list_from_json = json.load(input_file)
            self._data = self.convert_from_string(dictionary_list_from_json)
            if not self._data:
                print("ERROR: Data is still empty after conversion!")

    def write_to_file(self):
        with open(self._file, 'w') as outfile:
            json.dump(self.convert_to_string(self._data), outfile, indent=5)
